 ...
 *======================================================================================*/
	 ...
	 *
	 * In this class we check whether the session {@link kDEFAULT_SESSION} offset has
	 * received the session {@link CSessionObject object}, if that is not the case, the
	...
	protected function _InitResources()
	{
		...
		//
		if( (! isset( $_SESSION ))
		 || (! array_key_exists( kAPI_SESSION_SERVER, $_SESSION ))
		 || (! ($_SESSION[ kAPI_SESSION_SERVER ] instanceof CMongoServer)) )
			throw new Exception( "Session object not set or invalid",
								  kERROR_STATE );								// !@! ==>
 ...
 *======================================================================================*/
	 ...
	 * Parse request.
	 *
	 * We overload this method to parse the {@link kAPI_OPT_NO_RESP} tag.
	 *
	 * @access protected
	 *
	 ...
	 * have been sent.
	 *
	 * In this class we check if the provided {@link kAPI_OBJECT object} contains the
 ...
 *======================================================================================*/
	 ...
	 * @access protected
	 *
	 * @see kAPI_REQUEST kAPI_OPT_NO_RESP
	 */
	protected function _ParseNoResponse()
	{
		...
		if( array_key_exists( kAPI_OPT_NO_RESP, $_REQUEST ) )
		{
			if( $this->offsetExists( kAPI_REQUEST ) )
				$this->_OffsetManage
					( kAPI_REQUEST, kAPI_OPT_NO_RESP, $_REQUEST[ kAPI_OPT_NO_RESP ] );
		}
 ...
 *======================================================================================*/
	 ...
	 *
	 * In this class we set the database to a {@link CMongoDatabase} object.
	 *
	 * @access protected
	 *
	 * @see kAPI_SESSION_SERVER kAPI_SESSION_DATABASE kAPI_DATABASE
	 */
	protected function _FormatDatabase()
	{
		...
		if( array_key_exists( kAPI_DATABASE, $_REQUEST ) )
		{
			...
			//
			$_REQUEST[ kAPI_SESSION_DATABASE ]
				= $_SESSION[ kAPI_SESSION_SERVER ]
	 ...
	 *
	 * In this class we set the request container to a {@link CMongoContainer} object.
	 *
	 * @access protected
	 *
	 * @see kAPI_SESSION_DATABASE kAPI_SESSION_CONTAINER kAPI_CONTAINER
	 */
	protected function _FormatContainer()
	{
		...
		if( array_key_exists( kAPI_CONTAINER, $_REQUEST ) )
		{
			...
			if( array_key_exists( kAPI_DATABASE, $_REQUEST ) )
			{
				...
				//
				$_REQUEST[ kAPI_SESSION_CONTAINER ]
					= $_REQUEST[ kAPI_SESSION_DATABASE ]
						->Container( $_REQUEST[ kAPI_CONTAINER ] );
				
				//
				// Save container connection.
				//
	 ...
	 *
	 * In this class we set the query to a {@link CMongoQuery} object.
	 *
	 * @access protected
	 */
	protected function _FormatQuery()
	{
		...
		if( array_key_exists( kAPI_QUERY, $_REQUEST ) )
		{
			...
			// Handle match.
			//
			if( $_REQUEST[ kAPI_OPERATION ] == kAPI_OP_MATCH )
			{
				//
				// Create query array.
				//
				...
				foreach( $_REQUEST[ kAPI_QUERY ] as $query )
					$list[] = $_REQUEST[ kAPI_SESSION_CONTAINER ]->NewQuery( $query );
				
				//
				// Replace in request.
				//
				$_REQUEST[ kAPI_QUERY ] = $list;
	...
	protected function _FormatObject()
	{
		...
		if( array_key_exists( kAPI_OBJECT, $_REQUEST ) )
		{
			...
			if( array_key_exists( kAPI_CONTAINER, $_REQUEST ) )
			{
				...
				//
				if( $_REQUEST[ kAPI_CONTAINER ] instanceof CMongoCollection )
				{
					//
					...
					// Convert object.
					//
					$container->UnserialiseObject( $_REQUEST[ kAPI_OBJECT ] );
 ...
 *======================================================================================*/
	 ...
	 *
	 * This method can be used to check whether the provided
	 * {@link kAPI_QUERY query} parameter is valid.
	 *
			...
			if( array_key_exists( kAPI_CONTAINER, $_REQUEST ) )
			{
				...
				if( $_REQUEST[ kAPI_CONTAINER ] instanceof MongoCollection )
				{
					...
					if( $_REQUEST[ kAPI_OPERATION ] == kAPI_OP_MATCH )
					{
						...
						// Iterate queries.
						//
						foreach( $_REQUEST[ kAPI_QUERY ] as $key => $value )
 ...
 *======================================================================================*/
	...
	protected function _Handle_Count()
	{
		...
		$cursor = ( array_key_exists( kAPI_QUERY, $_REQUEST ) )
				? $_REQUEST[ kAPI_CONTAINER ]->find( $_REQUEST[ kAPI_QUERY ] )
				: $_REQUEST[ kAPI_CONTAINER ]->find();
		
		//
		// Set count.
		//
		$this->_OffsetManage( kAPI_STATUS, kAPI_AFFECTED_COUNT,
							  $cursor->count( FALSE ) );
	...
	protected function _Handle_GetOne()
	{
		...
		// Locate object.
		//
		$object = $_REQUEST[ kAPI_CONTAINER ]->findOne( $_REQUEST[ kAPI_QUERY ],
														$fields );
		...
		else
		{
			...
			// Set severity.
			//
			$this->_OffsetManage( kAPI_STATUS, kTERM_STATUS_LEVEL,
								  kSTATUS_WARNING );
	...
	protected function _Handle_Get()
	{
		...
		if( (! array_key_exists( kAPI_OPTIONS, $_REQUEST ))
		 || (! array_key_exists( kAPI_OPT_COUNT, $_REQUEST[ kAPI_OPTIONS ] ))
		 || (! $_REQUEST[ kAPI_OPTIONS ][ kAPI_OPT_COUNT ]) )
		{
			//
			// Handle results.
			//
			if( $count )
			{
				...
				if( $this->offsetExists( kAPI_PAGING ) )
				{
					...
					// Update parameters.
					//
					$this->_OffsetManage( kAPI_PAGING, kAPI_PAGE_START, $start );
					$this->_OffsetManage( kAPI_PAGING, kAPI_PAGE_LIMIT, $limit );
					$this->_OffsetManage( kAPI_PAGING, kAPI_RESP_PAGE_COUNT, $pcount );
...
//						$result = iterator_to_array( $cursor );
	...
	protected function _Handle_Match()
	{
		...
		// Handle query.
		//
		$queries = ( array_key_exists( kAPI_QUERY, $_REQUEST ) )
				 ? $_REQUEST[ kAPI_QUERY ]
				 : Array();
...
//						$result = iterator_to_array( $cursor );
	...
	protected function _Handle_Set()
	{
		...
		if( array_key_exists( kAPI_OPTIONS, $_REQUEST ) )
		{
			...
			// Iterate options.
			//
			foreach( $_REQUEST[ kAPI_OPTIONS ] as $key => $value )
			{
				//
				// Parse options.
				//
		...
		// Save object.
		//
		$ok = $_REQUEST[ kAPI_CONTAINER ]->save( $_REQUEST[ kAPI_OBJECT ], $options );
		$this->_OffsetManage( kAPI_STATUS, kAPI_STATUS_NATIVE, $ok );
		 ...
		 || (! $_REQUEST[ kAPI_OPT_NO_RESP ]) )
		{
			...
			// Which means that I cannot pass $this[ kAPI_RESPONSE ] to SerialiseData()
			// or I get the notice and the thing doesn't work.
			//
			$save = $this->offsetGet( kAPI_RESPONSE );
			CDataType::SerialiseObject( $save );
			$this->offsetSet( kAPI_RESPONSE, $save );
	...
	protected function _Handle_Update()
	{
		...
		$ok = $_REQUEST[ kAPI_CONTAINER ]->update
				( $query, $_REQUEST[ kAPI_OBJECT ], $options );
		$this->_OffsetManage( kAPI_STATUS, kAPI_STATUS_NATIVE, $ok );
		 ...
		 || array_key_exists( kAPI_OPT_FSYNC, $options ) )
		{
			...
			if( ! $ok[ 'ok' ] )
			{
				...
				// Set severity.
				//
				$this->_OffsetManage( kAPI_STATUS, kTERM_STATUS_LEVEL, kSTATUS_ERROR );
	 ...
	 * will update the provided object.
	 *
	 * The data provided in the {@link kAPI_OBJECT object} parameter will be scanned
	 ...
	 * {@link kAPI_OPTIONS options}, if not provided, it will default to <i>FALSE</i>.
	 *
	 * @access protected
	 */
	protected function _Handle_Delete()
	{
		...
		$ok = $_REQUEST[ kAPI_CONTAINER ]
				->remove( $_REQUEST[ kAPI_QUERY ], $options );
 ...
 *======================================================================================*/
	 ...
	 * {@link kAPI_OPTIONS options} are provided in the request.
	 *
